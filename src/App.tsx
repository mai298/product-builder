...
setProduct({
  ...
  const renderFormInputList = formInputList.map((formInput) => (
    <div className="flex flex-col" key={formInput.id}>
      <label htmlFor={formInput.id} className="mb-[2px] text-sm font-medium text-gray-700">{formInput.label}</label>
      <Input type="text" id={formInput.id} name={formInput.name} value={product[formInput.name]} onChange={onChangeHandler}/>
    </div>
  ));
